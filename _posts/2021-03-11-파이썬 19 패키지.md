...
`from game.sound.echo import echo_test`가 `from ..sound.echo import echo_test`로 변경되었다. 여기에서 `..`은 부모 디렉터리를 의미한다. graphic과 sound 디렉터리는 동일한 깊이(depth)이므로 부모 디렉터리(`..`)를 사용하여 위와 같은 import가 가능한 것이다.
