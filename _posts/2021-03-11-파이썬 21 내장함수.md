참고: https://wikidocs.net/32#enumerate
...
enumerate는 첫번째 인자로 반복 가능한 자료형 (리스트, 튜플, 문자열)을 입력으로 받아, 두번째 인자로 시작 인덱스를 받아, 인덱스 값을 포함하는 enumerate (열거형) 객체를 반환준다. 구문은 아래와 같다.

```python
enumerate(iterable, start=0)
```

아래는 네명의 이름을 받아서, 번호를 가진 튜플을 만드는 예이다. 

(3)에서 index는 0부터 시작하고, (5)에서 index는 1부터 시작한다.

```python
names = ['Michael', 'Daniel', 'John', 'Davis']
list(enumerate(names))
[(0, 'Michael'), (1, 'Daniel'), (2, 'John'), (3, 'Davis')]
list(enumerate(names, start=1))
[(1, 'Michael'), (2, 'Daniel'), (3, 'John'), (4, 'Davis')]
```



enumerate는 for문과 자주 사용되는데, for문의 수행 도중에 열거형 객체의 어느 위치에 해당하는지 알기 위해 인덱스가 필요한 경우 매우 유용하게 활용된다. 아래 예를 보면, 출력시 인덱스와 요소값이 같이 출력되어 자료형의 순서를 쉽게 알 수 있다. 
